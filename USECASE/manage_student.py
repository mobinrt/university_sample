...
class StudentUseCase(ObjectUseCase[StudentServices]):
    ...
    async def create_student(self, new_student: StudentCreate):
        ...
        db_student = await self.student_service.create_object(new_student, stu_id)
        self.unique_id.save_to_dict(db_student.name, db_student.id, ObjectToSTR.STUDENT.value)
        return db_student
            

    async def update_student(self, student_update: StudentUpdate, login_student: StudentModel):
        current_stu = await self.student_service.update_obj(student_update, login_student)
