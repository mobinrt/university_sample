...
async def create_teacher(teacher: TeacherCreate, teacher_service: TeacherServices = Depends(get_teacher_usecase)):
...
async def get_teacher_by_id(stu_id:int, teacher_service: TeacherServices = Depends(get_teacher_usecase)):
...
async def get_all_teachers(teacher_service: TeacherServices = Depends(get_teacher_usecase)):
...
async def update_teacher(
    ...
    teacher_service: TeacherServices = Depends(get_teacher_usecase)
    ):
...
async def delete_teacher_by_id(stu_id: int, teacher_service: TeacherServices = Depends(get_teacher_usecase)):
    await teacher_service.delete_by_id(stu_id)

@router.get('/get/hash_table/id', response_model=dict, status_code=status.HTTP_200_OK)
async def get_hash_table_id(teacher_service: TeacherServices = Depends(get_teacher_usecase)):
